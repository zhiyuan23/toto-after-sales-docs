...
项目统一按 3 个应用载体展示：Web 管理后台、消费者小程序和服务人员小程序。Web 后台共 63 个入口组，使用总部运营、客服作业、服务站作业、门店业务 4 个角色视角；历史 `Axx` 55 个、`Dxx` 8 个编号继续保留，但不再视为两个功能端。加上消费者 17 个、服务人员 16 个，仍为 96 个入口组。Web 后台的 A、D 编号入口、15 个功能模块及四视角访问关系统一维护在[Web 管理后台开发进度](03-开发管理/04-Web管理后台开发进度.md)。需求索引经核对包含 119 个正式需求编号和 45 个 `FS-*` 开发任务。菜单定义和需求索引不维护状态，当前状态统一从开发进度总览进入。

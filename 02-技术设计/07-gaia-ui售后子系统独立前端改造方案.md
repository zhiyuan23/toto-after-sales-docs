...
用户和角色属于统一体系。子系统权限管理必须覆盖当前操作者在主系统中真实可管理、可授予的完整 Gaia 范围，并使用与主系统相同的动态分类和权限目录；可以默认定位或筛选售后权限，但不能把静态售后清单当作完整权限树。可维护哪些用户、角色，可授予哪些模块和动作，均由 Gaia 后端校验，不以页面筛选替代。全局管理员跨模块管理沿用现有授权。保护既有管理员、共享角色及其他模块授权：编辑售后权限时不能用截断后的权限树全量覆盖角色，不能丢失未展示、未加载或当前操作者不可授予的授权；优先完整加载并提交 Gaia 要求的全部分类，或使用受范围约束的增量修改／后端安全合并，缺少保障时不得开放该写操作。

用户写入同样按完整聚合处理。Gaia 当前用户编辑可能同时维护组织关系、角色关系、头像、状态、品牌权限、组织数据权限和其他租户字段；子系统没有展示或没有修改的字段必须从最新详情安全保留，不能用精简表单或只读模型覆盖为空。所有 BIGINT 标识在前端继续以字符串传递；创建、编辑、启停、重置密码、删除、用户角色调整和角色模块／按钮授权均使用显式 DTO 白名单，并复用 Gaia 的密码规则、引用确认、当前用户／管理员保护、审计和事务语义。

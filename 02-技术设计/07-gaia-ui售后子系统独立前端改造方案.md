...
- 同源入口 `/after-sales/` 复用主系统 `localStorage.token` 与原始 Authorization 请求头；首次进入时获取菜单授权，同一 Token 的后续路由导航复用内存结果，Token 变化、退出或访问状态重试时失效。校验售后根、功能和视角的 code/path，过滤停用/隐藏菜单、空分组。管理员导航权限不替代功能权限。未授权直达、未登录、接口失败均关闭菜单；跨标签页更换账号或退出时清空菜单并重载。
...
### 2026-09-09 补充：会话内权限请求去重

- 菜单树和商品目录操作权限均按当前原始 Token 使用内存缓存；同一会话只在首次需要时加载，并发调用合并为一个请求。失败结果不缓存，页面重试仍会重新请求；Token 变化、退出、账号切换或整页刷新后重新校验。

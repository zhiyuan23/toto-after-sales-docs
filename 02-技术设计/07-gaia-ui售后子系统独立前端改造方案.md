...
- 版本：V1.4
- 更新日期：2026-09-10
- 文档状态：独立底座、统一运行／构建、菜单会话适配、独立登录及用户／角色可写代码已有实施记录；双端真实账号与权限验收、会话失效验证及三模式集成仍待完成
...
主系统原页面位于 `gaia-ui/src/views/afterSales/`；子系统位于 `gaia-ui/apps/after-sales/`，已有菜单会话适配、独立登录、用户／角色可写入口和部分真实业务页面。后续章节区分目标方案与历史实施记录，详见第 14 节；双端一致性、会话失效和真实权限验收尚未完成，不表示已经具备发布条件。
...
yarn dev:afs             # 清理旧进程后运行售后子系统及本地后端依赖
```
...
### 5.5 双登录与权限管理入口（2026-09-10 补充确认）

采用“前端入口独立、认证权限统一、后端保留独立部署空间”。主系统和子系统均可登录、创建用户、创建角色、维护用户角色关系和角色授权，上述认证、用户、角色和授权操作调用同一套 Gaia 后端并读写同一份平台数据；售后业务数据按 2026-09-11 确认的独立边界维护；不建设数据同步任务，不复制账号或权限表。

这里的“一致”和“同步”是同源读写，不是两套数据的最终一致性同步：两端使用相同记录 ID、租户、用户、角色、组织关系、模块、按钮和数据权限语义，任一端成功写入后，另一端重新查询必须直接得到同一记录的新状态。不得增加售后专用角色副本、映射表、定时任务或双写。两端的页面布局可以不同，但可见范围、可授予范围、保存结果和后端拒绝规则必须一致。
...
用户、角色、菜单及授权继续属于统一 Gaia 数据体系；子系统不复制表、不建立映射或同步任务。2026-09-16 确认售后入口采用受限管理视图：菜单只投影动态 `afterSales` 根及后代，售后根作为边界节点，不在子系统内移动、停用或删除；角色以服务端可校验的编码命名空间 `AFS_` 识别，名称、备注以及“当前恰好只拥有售后菜单”均不作为归属依据。子系统新增角色时由服务端生成 `AFS_` 加随机唯一值的编码，前端生成值只用于交互兼容，最终以服务端覆盖值为准。

角色列表、详情、新增、编辑和删除统一经过 `/api/afterSales/identity/roles` 受限接口。服务端读取操作校验当前操作者是否真实获授 Gaia `sys_role_list` 模块，新增、编辑和删除分别校验该模块下的 `add`、`edit`、`delete` 操作码，再强制校验目标角色编码以 `AFS_` 开头；不使用角色名称或旧全局接口 URL 的 `api_list` 命中结果代替模块／按钮授权。角色可授予的菜单与操作必须全部属于运行时 Gaia 目录中的 `afterSales` 子树，且至少选择一个售后菜单。非 `AFS_` 角色不会出现在售后列表，也不能通过该接口读取或修改；主系统原有角色及用户关系不会因本方案被改名、迁移或重写。历史售后角色只有在主系统明确将编码调整为 `AFS_` 后才进入售后管理范围，该动作不由子系统自动猜测或批量迁移。

“售后用户”不增加第二份用户记录，而定义为当前用户至少关联一个 `AFS_` 角色。用户列表、详情及写操作统一经过 `/api/afterSales/identity/users` 受限接口；读取校验 `sys_user_list` 模块授权，新增、编辑、启用、禁用、重置密码和删除分别校验该模块下对应操作码；新建用户必须至少选择一个售后角色。仅关联 `AFS_` 角色的账号可在售后入口维护完整 Gaia 用户聚合；同时关联其他系统角色的共享账号在售后入口只允许增删 `AFS_` 角色，服务端校验并原样保留全部非售后角色，且仅直接更新角色关系，不接受姓名、组织、数据范围、状态等全局字段变化。共享账号的启停、密码重置和删除一律拒绝并要求在主系统处理。前端的隐藏、禁用和提示只改善交互，最终边界由上述服务端校验和事务保证。

`AFS_` 是在现有 `sys_role` 无子系统归属字段条件下的过渡性强约束，不改变 Gaia 的全局可见性：主系统仍可查看和管理全部角色、用户，也会看到新建的 `AFS_` 角色；子系统只获得受限视图。主系统若绕过售后受限接口修改 `AFS_` 角色，仍应遵守只授予 `afterSales` 子树的约定，后续再次从售后接口保存时服务端会拒绝任何越出售后树的授权。若未来 Gaia 增加正式 `system_code`／成员关系字段，应通过显式迁移替换此前缀判定，不叠加第二套归属规则。

非共享售后用户写入仍按完整 Gaia 聚合处理。Gaia 当前用户编辑可能同时维护组织关系、角色关系、头像、状态、品牌权限、组织数据权限和其他租户字段；子系统没有展示或没有修改的字段必须从最新详情安全保留，不能用精简表单或只读模型覆盖为空。所有 BIGINT 标识在前端继续以字符串传递；创建、编辑、启停、重置密码、删除、用户角色调整和角色模块／按钮授权均使用显式 DTO 白名单，并复用 Gaia 的密码规则、引用确认、当前用户／管理员保护、审计和事务语义。共享用户按上一段的角色关系专用路径处理，不提交完整用户聚合。

主、子系统可能同时编辑同一用户或角色。实施前必须核对 Gaia 是否已有版本号、更新时间或其他并发检查；已有机制直接复用，没有机制时至少增加后端可验证的冲突保护，禁止后保存的一端静默覆盖先保存结果。角色基础信息与模块／按钮授权、用户基础信息与组织／角色／数据权限关系必须原子保存或在失败时完整回滚。
...
每个管理动作还必须同时核对 Gaia 菜单按钮的 `code`、`moduleCode`、`api_list` 和 HTTP Method。前端是否显示按钮只用于交互，最终由 `ApiPermissionCheckInterceptor` 及对应 AppService 的租户／管理范围校验拒绝越权；未配置进权限目录的写接口不能因“全局未命中则放行”而裸露。应使用允许账号和拒绝账号分别直调验证，而不是只看菜单是否隐藏。
...
当前独立登录及用户／角色只读入口已经完成，实施从现有代码增量推进，不重复搭建登录或另建管理页面：

1. 核对主系统、子系统、Compass 来源页面和 Gaia 管理后端；形成“主系统现有动作 → Gaia 接口／DTO／按钮与 API 权限 → 子系统 Compass 交互 → 保护规则”的完整矩阵，并先用只读证据确认全量保存、数据权限和并发语义。
2. 补齐当前用户和可管理范围获取，扩展既有用户只读页为 Compass 风格的新增、编辑、启停、重置密码和删除；完整处理组织、角色及 Gaia 实际存在的数据权限，不丢失未编辑字段。
3. 扩展既有角色只读页为 Compass 风格的新增、编辑、删除和模块／按钮授权；只有 Gaia 当前真实支持角色启停时才接入该动作。权限树动态读取 Gaia 完整可授予目录，保证非售后和未展示授权不被覆盖。
4. 补齐管理菜单与路由权限、每个动作的按钮权限、`api_list`、入口开关、后端管理范围、并发冲突和事务保护；恢复子系统真实菜单授权。
5. 验证账号切换、缓存清理、权限变更、禁用／删除／重置密码后的会话行为及既有售后业务回归。
6. 完成两端共同管理闭环：主系统创建／授权 → 子系统立即查询、登录和使用；子系统创建／授权 → 主系统立即查询和管理；任一端修改或撤权 → 两端重新查询一致且后端立即拒绝越权。抽查记录 ID、租户、组织、角色、模块、按钮和数据权限均为同一份数据。
7. 覆盖普通用户拒绝、管理员允许、入口关闭后直达拒绝、跨租户／超范围授权拒绝、隐藏／未加载权限保留、并发编辑冲突和会话失效；独立前端不加载主系统 HTML 即可登录管理。尚未实现的 iframe 集成明确列为未验证，不冒充完成。

隔离测试与共享环境验收分别记录。只有界面或构建通过不能标记真实权限完成；共享环境写入、正式账号授权和发布需另有明确授权。实施提示词见[双入口实施提示词](../specs/2026-09-09-统一用户角色只读管理阶段.md#dual-entry-checklist)。

### 5.10 2026-09-10 实施结果

已按第 5.9 节完成用户／角色可写代码：子系统复用 Gaia 原用户、角色、组织、动态模块／按钮目录、按钮操作码和密码规则接口；用户以最新完整详情构造非覆盖聚合 DTO，角色加载全部权限分类并全量保留已有授权。前端按 Compass 工作台、弹窗、危险确认、脏表单和权限选择器交互适配，不复制其 JWT 或 Resource RBAC。

Gaia 用户／角色聚合写服务增加事务；编辑请求继续携带已有 `updateTime`，后端据此拒绝主、子入口间的陈旧覆盖。前端 92 项测试、类型检查、生产构建和新增 SFC 实时编译通过，`gaia-sys-core` Maven 编译通过。共享环境账号、`api_list`、跨租户、允许／拒绝、双端实际写入及旧会话失效仍按第 5.9 节验收，不因代码完成提前标记闭环；详细证据见[统一用户与角色管理阶段](../specs/2026-09-09-统一用户角色只读管理阶段.md#6-2026-09-10-可写阶段实施)。
...
```json
{
  "scripts": {
    ...
    "dev:main": "vite --force",
    "dev:afs": "node scripts/frontends/dev-full.mjs after-sales",
...
售后业务服务在同一请求中可能连续执行入口权限和多个行操作权限判断。为避免每次判断都访问 Gaia 权限表，售后后端使用请求级 `GaiaAccessContext` 复用本请求权限集合，并在子系统内按 `userId + tenantId` 保存最长 30 秒的只读权限快照；缓存容量限制为 1024 个条目，到期后重新读取 Gaia 模块和操作授权。缓存不生成权限、不跨租户复用，未登录、缺租户和未授权继续失败关闭。

该措施只覆盖售后业务服务自己的权限读取，不替代 Gaia 聚合宿主的统一 API 权限拦截器。宿主拦截器或数据库网络仍可能构成请求时延下限；未经单独授权，不以售后子系统改动修改主系统拦截链。权限变更在售后业务服务中的最长可见延迟为 30 秒，撤权即时生效若成为正式要求，应增加定向缓存失效并专项验收。
...
- 修改或撤销权限后，Gaia 宿主权限链按平台规则生效；售后业务服务自身的权限快照最迟在 30 秒内刷新。若验收要求立即拒绝旧权限操作，须先补充定向缓存失效能力。
...
用户确认子系统不再显示重复的“售后服务”一级菜单。菜单数据直接使用工作台、客户服务等分组为第一层。子系统默认使用 `single` 单侧栏模式（一级与展开的二级均在同一左侧栏），工具栏导航按钮用于快速切换 `single` 与 `head` 顶部导航模式（顶部一级＋左侧二级）；`side` 左侧两栏模式仅保留在开发设置中。工具栏按操作习惯排列为导航搜索、刷新／全屏页面操作、导航模式／深浅色／语言显示偏好；原工具栏身份管理入口已移除，经 Gaia 真实授权投影的“系统管理”分组固定放在左侧菜单末尾。49项功能和全部工作视角分组配置本地打包的统一 Lucide 图标，首页入口与标签同步使用。原有顶部重复菜单消失、功能图标显示和临时关闭菜单权限的配置保持不变。

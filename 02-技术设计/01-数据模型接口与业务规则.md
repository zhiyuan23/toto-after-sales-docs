...
2026-09-18 阶段 1 源码核查发现当前物理实现已存在 `afs_service_personnel_station` 多站关系，且 `afs_service_personnel.station_id` 只同步关系顺序中的第一站。服务人员移动上下文为兼容现有数据，从所有有效关系站计算企业候选；这只是对已实现模型的事实记录，不把多站实现反写成已确认的主组织业务规则。后续若收敛为“唯一主属站＋独立支援关系”，需另行迁移并保持历史上下文可追溯。移动账号与会话设计见[服务人员移动身份与企业上下文](15-服务人员移动身份与企业上下文.md)。

...
> 2026-09-11 已确认：以下业务实体均归售后子系统独立持有；商品／分类／代理商／门店不映射到主系统业务主档，关系不得引用主系统业务 ID。Gaia 统一认证权限与租户平台保留，外部 DWH／追溯按单独接口契约接入。当前代码偏差见 [业务数据独立边界与核查](../specs/2026-09-11-业务数据独立边界与已开发功能核查.md)。
...
<a id="station-technician-model"></a>

#### 12.1.1 服务站与维修人员主数据关系

2026-09-12 用户确认：图中的 `SERVICE_STATION ||--o{ TECHNICIAN` 表示 **1:N 主组织关系**。一个服务站可管理多个维修人员，每位维修人员有且只有一个主属服务站；不得用服务站与维修人员的多对多绑定表示主组织归属。业务说明及管理依据见[服务站与维修师傅关系](../01-功能需求/02-角色权限与总体架构.md#611-服务站与维修师傅关系)。

- 维修人员主档使用单值的主属服务站关联表达组织归属，并校验其有效性；具体物理表名、字段名和迁移由对应功能契约确认。
- 可服务区域、可支援服务站及临时调度作为独立的后续扩展关系或调度记录，不复用主属服务站字段存储多个站，也不通过修改主属站实现临时支援。
- 工单责任服务站与执行维修人员分别记录；跨站执行时，工单责任服务站可以与维修人员主属服务站不同。原“统一工单”中的 `station_id` 表示工单责任服务站，`technician_id` 表示实际执行人员，不能用后者的主属站反推或覆盖前者。
- 扩展跨站支援时，应能区分工单责任站、执行人、执行时的人员主属站及派单类型，并保留授权与调度记录，使派单权限、责任、绩效、费用和统计可追溯。跨站执行本身不改变主属服务站；具体历史记录方式和核算规则待功能契约确认。

本节是已确认的目标约束及扩展边界，不表示跨站支援或临时调度已经实现，也不要求本次新增扩展表、接口或迁移。

2026-09-21 已按上述规则完成实现收敛：`afs_service_personnel.station_id` 是人员主属服务站的唯一业务依据，人员、排班、库存、派单、服务站统计及移动上下文查询均只读该字段。历史 `afs_service_personnel_station` 不再表达多站归属，仅作为单行兼容镜像随人员保存同步；增量迁移在确认全部镜像与主站一致后增加 `personnel_id` 唯一约束，并将 `ordinal` 固定为 1。旧客户端若仍提交 `stations` 数组，后端只在缺少 `stationId` 时读取第一项，保存后收敛为唯一主站；新客户端只提交单值 `stationId`。跨站支援仍未实现，未来必须使用独立授权／调度记录。移动账号与会话设计见[服务人员移动身份与企业上下文](15-服务人员移动身份与企业上下文.md)。
...
| product | id、code、barcode、model、base_model、tank_model、specification、color、category_id、series_id、enabled | 售后独立商品主档，对应 `afs_product`，不关联 `md_material` |

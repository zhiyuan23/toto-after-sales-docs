...
当前导航以[后台菜单与角色权限手册](09-后台菜单与岗位权限矩阵.md)为准。程序已接入四个工作视角、16 个展示分组（2026-09-12 源码快照）、管理员全局切换、授权菜单投影、默认首页与直达校验；原有业务域仅保留为唯一注册和代码存储结构。总部视角将配件档案放在“商品管理”分组，“配件管理”从库存台账开始；服务站没有“商品管理”分组，仍将配件档案放在“配件管理”首位。V3 实施范围及验证边界见[后台菜单与工作视角收敛记录](../specs/2026-09-11-后台菜单与工作视角收敛.md)。
...
| 消费者客服渠道配置 | 售后服务／总部运营／运营管理／客服渠道配置（`afsConsumerSupportConfig`） | A51 | 电话与微信渠道需要租户隔离、字段校验、版本和变更历史，使用售后专属表与接口 |
| 其他通用业务配置 | 蓝鲸数字全局系统配置 | A51 | 继续复用全局字典、通用参数和通知配置能力；需要售后业务语义时再按确认范围接入 A51 页面 |
| 日志审计 | 蓝鲸数字全局日志管理 | A52、A53 | 复用全局操作日志与登录日志能力 |

角色、用户和日志入口继续由蓝鲸数字全局能力承接。消费者客服渠道配置使用客户服务存储域下的 `afsConsumerSupportConfig / afterSales/customerService/supportChannel` 页面，运行时仅投影到总部运营的“运营管理”分组；不得借此复制全局系统管理页面。
...
9. 展示顺序由工作视角清单维护，遵循高频任务优先、查询早于资料维护、维度先于主体、单据按业务发生顺序。总部“商品管理”按商品分类、商品系列、商品档案、配件档案排列，“配件管理”从库存台账开始；服务站“配件管理”仍按配件档案、库存台账及业务单据排列。同一功能可在不同视角投影到适合该岗位的分组，但保持唯一 code、path 和页面。
...
生成结果为 1 个售后根节点、11 个稳定存储分组、51 个唯一功能节点和 5 个隐藏导航授权节点，共 68 个当前节点；其中 40 个功能进入当前工作视角侧栏、1 个为上下文功能、10 个为未被工作视角引用的隐藏规划功能。同时区分只允许人工复核后退役的 legacy 候选，以及本次必须保持不变的根系统兼容节点。只读预检 SQL 核对现有模块、父级、启停、隐藏状态、角色绑定、操作和 API，不包含写语句。工作视角的 16 个展示分组仍由前端投影，不在数据库复制同一功能。

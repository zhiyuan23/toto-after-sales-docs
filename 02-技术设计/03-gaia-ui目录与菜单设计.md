...
> 架构演进说明：本文记录当前已实施的主 SPA 菜单、路径和页面目录。已确认的目标方案将售后 Web 迁移到同一 `gaia-ui` 仓库的 `apps/after-sales/` 独立 SPA，菜单 code、角色投影和 Gaia 权限来源保持不变；源码、运行、构建及部署改造见[售后子系统独立前端改造方案](07-gaia-ui售后子系统独立前端改造方案.md)。迁移验收完成前，本文所列 `src/views/afterSales/` 仍代表当前实现。

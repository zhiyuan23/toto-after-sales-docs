...
当前导航以[后台菜单与角色权限手册](09-后台菜单与岗位权限矩阵.md)为准。2026-09-25 已按岗位任务重排四视角：总部商品资料先于配件库存，服务站人员排班先于配件库存，服务知识库归入作业支持；四首页与门店常用业务一级直达。系统管理进入正常菜单末尾，导入导出任务在工作台右上角以宽抽屉打开。导入导出配置后续收进“菜单与权限”详情页签，只允许管理员按独立配置权限使用；模板仍在业务配置，任务已移入工作台宽抽屉。显示层级不改变功能 code、路径、稳定权限父级及业务授权。任务交互见[工作台任务抽屉](../specs/2026-09-25-工作台导入导出任务抽屉.md)，菜单结构详见[菜单重排实施记录](../specs/2026-09-25-四视角菜单结构与排序调整.md)。
...
| 导航层级 | 当前视角 → 一级直达入口或业务分组 → 功能菜单；详情、编辑、扫码填写、签名等作为隐藏路由或页面内流程 |
...
| 菜单来源 | 继续由 `/api/sys/Module/tree` 返回授权；独立子系统通过 `src/router/routes.ts` 将稳定功能 code 映射到实际页面文件，源码目录不要求与菜单 path 相同 |

这里的“代理商工作台”统一称为门店业务视角，是同一 Web 管理后台中的角色门户。历史 `Dxx` 入口可以单独筛选和验收，但项目总进度必须汇总到 Web 管理后台，不再作为独立功能端统计。

### 当前源码目录与稳定访问路径（2026-09-25）

独立子系统按业务职责整理了以下六项、七个功能目录。源码路径从 `apps/after-sales/src/views/` 起算，页面访问 URL、稳定菜单 code、权限分组、API 与角色授权不随此次目录移动变更。

| 功能 | 当前源码目录 | 保留的访问路径 |
| --- | --- | --- |
| 评价标签配置 | `businessConfig/evaluationTags` | `/memberOperation/serviceSurvey` |
| 标签管理 | `businessConfig/tag` | `/memberOperation/tag` |
| 字典管理 | `businessConfig/dictionary` | `/dataCenter/dictionary` |
| 客服渠道配置 | `businessConfig/supportChannel` | `/customerService/supportChannel` |
| 安装码查询 | `customer/installationCode` | `/antiDiversion/installationCode` |
| 运营报表 | `analytics/report` | `/dataCenter/report` |
| 服务质量 | `analytics/quality` | `/serviceResource/quality` |

四视角通过同一功能 code 复用这些页面，不按视角复制源码；内部组件、model、schemas 随功能迁移。`downloadCenter`、`catalog`、`inventory`、`network` 等目录保持现有职责。后文稳定注册路径不等于源码目录；当前映射以 [`routes.ts`](../../../frontend/gaia-ui/apps/after-sales/src/router/routes.ts) 为准。实施与验证见[六项目录整理](../specs/2026-09-25-菜单稳定后六项前端目录整理.md)。

## 2. 命名原则

1. 一级菜单使用通用 SaaS 领域名称“**售后服务**”，不包含具体租户或品牌名称，也不使用“管理后台”“代理商系统”等技术或角色名称。
2. 导航分组按岗位任务组织，例如“服务运营、工单处理、本站工单、人员排班”；功能 code 和 path 保持稳定，用户名称可按业务理解收敛。
...
本节保留 2026-09-08 的 V2 目录用于差异追溯，不再作为当前导航实现依据。V3 当前目录以[角色权限手册第 2 节](09-后台菜单与岗位权限矩阵.md#full-catalog)和独立子系统 `src/features/navigation/manifest.json` 为准：管理员切换器位于全局页头，侧栏显示当前视角的一级直达入口和“分组 → 功能”，不存在第五套管理员混合菜单。
...
| 分组／商品管理 | `afsCatalog` | `afterSales/catalog` | 运行导航按视角放入“商品管理”“业务查询”或“配件管理” |
| 分组／服务资源 | `afsServiceResource` | `afterSales/serviceResource` | 服务人员、排班、质量和服务站工单分配 |
| 分组／配件管理 | `afsInventory` | `afterSales/inventory` | 服务站、服务人员两级库存和单据闭环（无仓库） |
...
| 菜单名称 | 菜单代码 | path／页面目录 | 入口 ID | 建议角色 |
| --- | --- | --- | --- | --- |
| 配件入库 | `afsPartInbound` | `afterSales/inventory/inbound` | A30 | 管理员、总部、服务站管理员 |
...
### 4.11 蓝鲸数字全局系统能力与售后专属配置

| 能力 | 承接位置 | 入口 ID | 说明 |
| --- | --- | --- | --- |
| 角色与权限 | 蓝鲸数字全局权限管理 | A46、A47 | 配置售后相关角色的菜单、按钮、接口和数据范围，不在 `afterSales` 下重复建设 |
| 用户管理 | 蓝鲸数字全局用户管理 | A48、A49、A50 | 复用全局用户、组织和账号能力 |
| 消费者客服渠道配置 | 售后服务／总部运营／业务配置／客服渠道配置（`afsConsumerSupportConfig`） | A51 | 电话与微信渠道需要租户隔离、字段校验、版本和变更历史，使用售后专属表与接口 |
| 其他通用业务配置 | 蓝鲸数字全局系统配置 | A51 | 继续复用全局字典、通用参数和通知配置能力；需要售后业务语义时再按确认范围接入 A51 页面 |
| 日志审计 | 蓝鲸数字全局日志管理 | A52、A53 | 复用全局操作日志与登录日志能力 |

角色、用户和日志入口继续由蓝鲸数字全局能力承接。消费者客服渠道配置使用客户服务存储域下的 `afsConsumerSupportConfig / afterSales/customerService/supportChannel` 页面，运行时仅投影到总部运营的“业务配置”分组；不得借此复制全局系统管理页面。

## 5. gaia-ui 页面目录

以下为历史 legacy 目录方案，用于路径追溯；当前实现使用 `apps/after-sales/src/views/`，本次展示分组调整不移动业务源码目录。
...
9. 展示顺序由工作视角清单维护，按岗位职责、查询／办理频率及任务关系排序；不以建档依赖顺序代替日常优先级。总部商品档案先于系列和分类，商品资料先于配件库存；服务站人员排班先于配件库存；总部与服务站的配件库存共有入口统一为库存台账、配件入库、领料管理、退料管理、工单耗用、调拨管理、盘点管理，服务站配件档案置末。工作台和门店业务一级直达；系统管理为最后一个普通分组，导入导出任务在工作台右上角以宽抽屉打开。

## 7. 角色默认导航与授权来源

| 角色 | 工作视角 | 默认首页 | 功能数 |
| --- | --- | --- | ---: |
| 系统管理员 | 已有业务授权对应的视角 | 当前视角工作台；页头可切换视角 | 当前 43 个去重导航注册＋2 个上下文功能，实际业务逐项授权 |
| 总部管理／运营 | 总部运营 | 运营工作台 | 当前目录 27 个侧栏功能；另有工作台内任务工具，购买登记写操作及配置维护按实际职责另授 |
| 客服主管 | 客服作业 | 客服工作台 | 最多 14 个导航功能＋1 个上下文入口 |
| 客服专员 | 客服作业 | 客服工作台 | 导航按实际功能授权收窄，另有 1 个上下文入口 |
| 服务站管理员 | 服务站作业 | 服务站工作台 | 最多 19 个导航功能＋1 个上下文入口 |
| 代理商管理员、店员 | 门店业务 | 门店工作台 | 各 5 个导航功能；服务工单只开放授权范围内的查看与另授动作 |
...
| 主档管理 | 按岗位拆入“渠道与网点”“商品管理”“人员与资料”等分组 | 减少单组菜单数量，名称更贴近日常管理对象 |
...
| 系统管理中的角色、权限、三类用户、两类日志 | 蓝鲸数字全局系统能力承接 | 售后服务不重复建设全局身份与日志入口；消费者客服等售后专属参数进入总部运营“运营管理”下的 A51 客服渠道配置 |
| Excel 导入导出 | 数据任务 | 统一承载异步任务、错误明细和文件生命周期 |

## 9. Gaia 菜单配置与分发

业务目录与角色规则统一见[后台菜单与角色权限手册](09-后台菜单与岗位权限矩阵.md)。独立子系统当前代码映射位于 `apps/after-sales/src/features/navigation/manifest.json`；56 个当前功能 code/path 和 5 个导航授权 code 由该清单集中维护。`afsDealerAppointment`、`afsDecorationCompany`、`afsServiceItem`、`afsProductBundle` 已退出当前注册范围；`afsInstallationCode` 已按明确要求恢复，A51 客服渠道配置已进入总部运营“业务配置”分组。菜单交付生成器必须直接读取这份独立子系统清单，不得读取 legacy 常量或据此恢复已删除功能及 V2 全量侧栏。
...
生成结果为 1 个售后根节点、11 个稳定存储分组、56 个唯一功能节点和 5 个隐藏导航授权节点，共 73 个当前节点；43 个功能进入日常导航、2 个作为上下文功能、11 个保留在导航外（包括管理员菜单详情中的导入导出配置）。四视角 16 个普通展示分组及一级直达入口只由前端投影，不在数据库复制功能。退役候选与根系统兼容节点继续区分处理。

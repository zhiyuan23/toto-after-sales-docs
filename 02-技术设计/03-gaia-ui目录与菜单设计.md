...
当前导航以[后台菜单与角色权限手册](../04-业务阅读/05-后台菜单与角色权限手册.md)为准。程序已接入四个工作视角、14 个展示分组、管理员全局切换、授权菜单投影、默认首页与直达校验；原有 10 个业务域仅保留为唯一注册和代码存储结构。V3 实施范围及验证边界见[后台菜单与工作视角收敛记录](../specs/2026-09-11-后台菜单与工作视角收敛.md)。
...
以上是 V2 历史数量。当前 V3 为 4 个工作视角、14 个展示分组、52 处侧栏引用，去重后 33 个当前侧栏功能；另有 1 个服务侧安装申请查询上下文功能和 11 个规划隐藏功能。当前注册功能共 45 个；A05、A13、A38、D08 仅保留历史编号用于变更追溯。

## 4. 菜单节点、路径和需求入口映射

下表按原有存储目录列出 45 个当前唯一功能注册，**不表示运行侧栏分组，也不表示当前全部可见**。角色列保留功能分配方向；V3 是否进入侧栏／上下文／规划隐藏以导航清单和角色权限手册为准。查看不包含写操作，管理员仍受功能授权、租户和业务数据范围限制。
...
A38 独立“安装码管理”已从当前 Web 功能注册、路由和页面删除。安装码仍由购买登记生成，并继续作为顾客购买记录、服务受理和工单查询／详情中的识别与追溯字段，不单独形成管理入口。
...
| 角色 | 工作视角 | 默认首页 | 功能数 |
| --- | --- | --- | ---: |
| 系统管理员 | 全部四个 | 首个有权功能；系统首页或页头可切换视角 | 当前 33 个侧栏功能＋1 个上下文功能的授权上限 |
| 总部管理／运营 | 总部运营 | 运营概览 | 最多 21 个侧栏功能 |
| 客服主管 | 客服作业 | 服务受理 | 最多 12 个侧栏功能＋1 个上下文入口 |
| 客服专员 | 客服作业 | 服务受理 | 默认最多 10 个侧栏功能＋1 个上下文入口 |
...
多角色账号合并功能授权，在当前视角下去重显示；普通业务账号当前按首个有效岗位视角进入，不展示管理员专属切换器。首页无权时落到第一个授权功能，空视角不显示；直达某个有权功能时恢复包含该功能的视角。越权功能转入访问状态页。

管理员可在系统首页直接更改当前菜单视角，也可从全局页头切换并进入该视角默认功能；无默认功能权限时进入第一个授权功能。视角不当作授权缓存，登录后重新核对菜单。组织／门店切换、待办数量和业务数据范围联动尚待各切片完成，当前选择器只切导航，不改变后端身份。
...
| 安装码管理 | 删除独立功能；安装码并入相关记录检索与详情 | 安装码是购买商品实例的识别字段，不单独维护一套只读查询页面 |
| 标签管理单独成组 | 并入总部“运营管理” | 单功能不再独占一级展示分组；其他会员功能完成前保持导航隐藏 |
...
业务目录与角色规则统一见[后台菜单与角色权限手册](../04-业务阅读/05-后台菜单与角色权限手册.md)。独立子系统当前代码映射位于 `apps/after-sales/src/features/navigation/manifest.json`；45 个当前功能 code/path 和 5 个导航授权 code 由该清单集中维护。`afsDealerAppointment`、`afsDecorationCompany`、`afsServiceItem`、`afsInstallationCode` 已从注册与路由删除；旧生成脚本只用于既有 Gaia 节点核对，不得据此恢复已删除功能或 V2 全量侧栏。

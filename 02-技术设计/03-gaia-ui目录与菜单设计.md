...
9. 展示顺序由工作视角清单维护，按岗位职责、查询／办理频率及任务关系排序；不以建档依赖顺序代替日常优先级。总部商品档案先于系列和分类，商品资料先于配件库存；服务站人员排班先于配件库存，领退料先于库存资料维护。工作台和门店业务一级直达；系统管理为最后一个普通分组，导入导出任务在工作台右上角以宽抽屉打开。

...
该命令会先关闭上一轮仍占用 7004/9010 的本工作区 Vite 进程，再按需启动或复用 `127.0.0.1:16379` Redis、装配售后模块并启动 `127.0.0.1:8080` Gaia 聚合后端，最后启动 7004 主系统和 9010 售后子系统前端。若端口属于其他程序，命令会列出 PID 并拒绝强杀。后端首次装配通常需要约 1–2 分钟；出现“已就绪”日志后访问 `http://127.0.0.1:7004/`。`Ctrl+C` 只停止本次命令新建的进程，不会关闭启动前已经运行的 Redis 或后端。该入口依赖当前跨仓目录结构以及本机 bash、JDK 21、Maven、Redis、lsof，仅支持 macOS/Linux。

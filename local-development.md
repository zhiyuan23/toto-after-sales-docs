...
该命令会先关闭上一轮仍占用 7004/9010 的本工作区 Vite 进程，再调用 `scripts/local-backend.mjs`：同时具备 Web 与移动端能力的 `127.0.0.1:8080` Gaia 聚合后端直接复用；没有后端时，以跨项目启动锁只装配并启动一次；端口已被不兼容服务占用时明确报错并拒绝另开端口。最后启动 7004 主系统和 9010 售后子系统前端。后端首次装配通常需要约 1–2 分钟；出现“已就绪”日志后访问 `http://127.0.0.1:7004/`。共享 Redis 和后端独立常驻，`Ctrl+C` 只停止本次前端，不影响同时运行的两个小程序。该入口依赖当前跨仓目录结构以及本机 bash、JDK 21、Maven、Redis、lsof，仅支持 macOS/Linux。
...
若后台、小程序同时启动，最先取得启动锁的项目负责拉起后端，其他项目等待健康检查后复用。退出任一前端都不关闭共享后端。仅显式传入 HTTPS `GAIA_API_BASE_URL` 时按远端联调处理，不启动本地后端。

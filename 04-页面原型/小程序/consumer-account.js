...
(() => {
  ...
  function records(ctx) {
    ...
    for (const product of ctx.hasProducts===false?[]:ctx.products || []) {
      ...
      if (!groups.has(key)) groups.set(key,{id:key,date:info.purchaseDate || '',store:info.storeName || '',source:info.method==='existing-record'?'手机号同步':info.method==='scan-credential'?'凭证码扫码':info.method==='scan-unique'?'实物码扫码':'用户申报',verified:['existing-record','scan-credential','scan-unique'].includes(info.method),products:[]});
      groups.get(key).products.push(product);
    }
    return [...groups.values()].sort((a,b)=>b.date.localeCompare(a.date));
  }
  const empty = () => '<div class="c-account-empty"><span class="c-empty-mark" data-icon="service" aria-hidden="true"></span><h2>还没有同步到购买记录</h2><p>先授权购买时使用的手机号，<br>系统会自动检查已接入的订单。</p><button class="primary" data-go="c-phone-sync">授权手机号并同步</button><button class="c-text-button" data-go="c-register">扫一扫添加其他产品 ›</button></div>';
  const purchaseBody = ctx => {
    const list=records(ctx);
    if (!list.length) return empty();
    return `<p class="c-list-summary">共 ${list.length} 条记录 · ${list.reduce((sum,r)=>sum+r.products.length,0)} 件产品</p><div class="c-purchase-list">${list.map(record=>`<article class="c-purchase-card"><header><div><span>购买日期</span><h3>${e(record.date || '尚未提供')}</h3></div><span class="c-tag">${e(record.source)}</span></header><p class="c-purchase-store">${e(record.store || '购买门店未提供')}</p>${record.products.map(p=>`<button class="c-record-product" data-product="${e(p.id)}" data-go="c-product"><img src="${e(p.image)}" alt="${e(p.name)}"><span><strong>${e(p.name)}</strong><small>${e(p.model)}</small></span><span class="c-record-quantity">1 件<span aria-hidden="true"> ›</span></span></button>`).join('')}<button class="c-record-expand" data-account="record" data-record-id="${e(record.id)}" aria-expanded="${expanded.has(record.id)}" aria-controls="purchase-${e(record.id)}"><span>来源与产品信息</span><span aria-hidden="true">${expanded.has(record.id)?'收起 −':'查看 +'}</span></button>${expanded.has(record.id)?`<div id="purchase-${e(record.id)}" class="c-record-details"><p>来源：${e(record.source)}（原型示例）${record.verified?'':'，购买与实物尚未核验'}</p>${record.products.map(p=>`<div><strong>${e(p.name)} · ${e(p.room)}</strong>${p.installationCodeLinked===true && p.installationCode?`<span class="c-record-code">${e(p.installationCode)}</span><button class="c-text-button" data-action="演示复制：${e(p.installationCode)}。未写入系统剪贴板。">复制安装码</button>`:'<span class="c-record-code c-secondary-text">暂无关联安装码</span>'}</div>`).join('')}<p>产品关系与服务权益分开判断，申请服务时仍需核验。</p></div>`:''}</article>`).join('')}</div><p class="c-hint">手机号同步与扫码只展示当前账户可见记录；用户申报不代表已经核验。</p><button class="c-menu-row" data-go="c-phone-sync"><span>重新同步购买记录</span><span aria-hidden="true">›</span></button>`;
  };
  ...
  screens.splice(screens.findIndex(s=>s.id==='c-mine')+1,0,
    ...
    {id:'c-purchases',title:'购买记录',entry:'C11 · 自动同步与其他来源',parent:'c-mine',goal:'按购买事实汇总手机号同步、凭证码扫码和用户申报记录，并保持来源与核验状态可辨认。',note:'CAPP-012 设计草案。手机号同步只匹配已验证手机号与已接入可信订单；不按姓名或模糊日期猜测。用户申报单独展示且不生成权益。数量按产品实例计；示例不证明真实授权或服务权益。',body:purchaseBody}
  );
})();

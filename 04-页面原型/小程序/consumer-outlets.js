...
(() => {
  ...
  const initial = () => ({kind:'repair',city:'上海市',district:'全部区县',view:'map',selectedId:'r-xh',location:'idle',locationOutcome:'success',failed:false,pickCity:'上海市',pickDistrict:'全部区县',overlay:''});
  let state = initial(), focusTarget = '';
  const results = () => outlets.filter(o => o.kind===state.kind && o.city===state.city && (state.district==='全部区县' || o.district===state.district));
  ...
  const note = '地图优先：顶部只保留维修网点／授权门店切换，下方展示选中网点和电话、导航；点名称查看详情，展开列表选其他网点。仅在定位失败或空结果时按需进入地区选择。名称、地址、电话、时间、距离及位置均为虚构演示资料。定位、拨号与微信位置页仅模拟；HTML 不调用真实微信接口，也不支持真实地图拖动或缩放。';
  ...
  function controls() {
    return `<div class="o-top-controls"><div class="o-type-switch" aria-label="网点类型">${Object.entries(kinds).map(([key,kind])=>`<button data-outlet="kind" data-value="${key}" aria-pressed="${state.kind===key}">${kind.name}</button>`).join('')}</div></div>`;
  }
  ...
  function directory() {
    ...
    const empty = `<div class="o-empty">${icon('search')}<h3>这里暂未找到${kinds[state.kind].name}</h3><p>可以更换地区后再查看</p><button class="primary" data-outlet="region">更换城市</button><button class="c-text-button" data-action="客服渠道待配置；本次原型不连接真实客服。">联系 TOTO 客服</button></div>`;
  ...
  app.screens.push(
    {id:'c-outlets',parent:'c-service',title:'查找服务网点',entry:'C08 · 地图查找',goal:'先在地图找到位置，再从底部卡片直接联系或导航；顶部仅保留网点类型切换，详细资料按需查看。未登记产品也可查询。',note,body:directory},
  ...
  window.TOTO_OUTLETS = {
    ...
    handle:(el,api)=>{
      ...
      if(action==='kind' && kinds[value]) {state.kind=value;state.selectedId='';}
      ...
      else if(action==='clear'){state.district='全部区县';state.selectedId='';}

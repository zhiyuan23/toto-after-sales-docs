...
(function () {
  ...
  const REVIEW_TIME='13:00'; // Fixed review clock; never a production scheduling source.
  ...
  const seedTasks = () => [
    ...
    {id:'ZD202609130008',name:'许先生',type:'远程指导',queue:'pending',flag:'待联系',date:TODAY,start:'15:00',end:'17:00',address:'徐汇区 · 远程服务',product:'智能坐便器',model:'DEMO-T01',issue:'需要了解遥控器设置。'},
    ...
    {id:'WX202609120099',name:'测试客户',type:'维修',queue:'pending',flag:'已预约',date:'2026-09-12',start:'09:00',end:'11:00',address:'徐汇区 · 预约核对示例地址',product:'智能坐便器',model:'DEMO-T01',issue:'预约时间已过，需要核对安排。'},
    ...
    {id:'AZ202609100002',name:'孙女士',type:'安装',queue:'followup',flag:'审核通过',date:'2026-09-10',completedAt:'2026-09-11',start:'09:00',end:'11:00',address:'徐汇区 · 示例华庭',product:'智能坐便器',model:'DEMO-T01',issue:'安装已完成。',versions:[{n:1,result:'安装完成',status:'审核通过',note:'审核通过，后续由服务中心跟进'}]},
  ...
  function reset(company='上海示例服务企业') {
    state={company,loggedIn:true,tasks:company==='上海示例服务企业'?seedTasks():[],taskId:'WX202609130018',queue:'pending',search:'',sort:'priority',filter:'all',step:0,listPosition:null,scheduleDate:TODAY,
  ...
  const taskCardScenario = () => activeTasks().length>1?'multiple':activeTasks().length===1?'single':'overview';
  function setTaskCardScenario(value){
    if(!['overview','single','multiple'].includes(value))return;
    reset();
    const ids=value==='multiple'?['WX202609130018','AZ202609130021']:value==='single'?['WX202609130018']:[];
    ids.forEach(id=>{const t=state.tasks.find(item=>item.id===id);t.signed=true;t.arrival='示例位置签到 · 09-13 14:02';startService(t);log(t,'到达签到已记录','仅示例位置，未读取真实定位。');});
  }
  ...
  const history = t => `<div class="w-timeline">${[...t.history].reverse().map(h=>`<div><strong>${e(h.title)}</strong><p>${e(h.note)}</p><small>${e(h.time)}</small></div>`).join('') || '<div><strong>暂无联系与处理记录</strong></div>'}</div>`;
  const log = (t,title,note) => t.history.push({title,note,time:'09-13 · 本次演示'});
  const workGroup=t=>!inQueue(t,'pending')?'':(['缺件','断联'].includes(t.flag)||t.queue==='lost')?'WAITING':!['已预约','改期'].includes(t.flag)||!t.date?'CONTACT':!isServing(t)&&(t.date<TODAY||(t.date===TODAY&&(t.end||t.start)&&(t.end||t.start)<REVIEW_TIME))?'OVERDUE':t.date===TODAY?'TODAY':'FUTURE';
  const workGroupLabels={TODAY:'今日已确认预约',CONTACT:'待联系确认',FUTURE:'未来已确认预约',WAITING:'等待配件 / 待再次联系',OVERDUE:'预约时间已过 · 需核对安排'};
  const appointmentLabel=t=>workGroup(t)==='WAITING'?(t.date?'原时间 · 待重新确认':'待重新预约'):['已预约','改期'].includes(t.flag)&&t.date?'已确认预约':t.date?'客户期望 · 待确认':'待预约';
  const displayDate=t=>t.date===TODAY?'今天':t.date==='2026-09-14'?'明天':t.date?.slice(5).replace('-','月')+'日';
  function filteredTasks(queue=state.queue) {
    ...
    if(state.filter==='overdue')list=list.filter(t=>workGroup(t)==='OVERDUE');
    ...
    const ranks={TODAY:0,CONTACT:1,FUTURE:2,WAITING:3,OVERDUE:4};
    return list.sort((a,b)=>queue==='pending'?(ranks[workGroup(a)]-ranks[workGroup(b)]||(a.date+a.start||'9999').localeCompare(b.date+b.start||'9999')||a.id.localeCompare(b.id)):(b.reviewedAt||b.submittedAt||b.completedAt||b.date||'').localeCompare(a.reviewedAt||a.submittedAt||a.completedAt||a.date||''));
  }
  ...
  function nextStep(t) {
    ...
    if(workGroup(t)==='OVERDUE')return {label:'需核对安排',hint:'预约时间已过，请核对是否需要改约或补记服务进度。',action:'核对安排',route:'w-detail',word:'核对'};
    if(t.date&&!['已预约','改期'].includes(t.flag))return {label:'待联系确认',hint:'这是客户期望时间，请联系客户确认预约。',action:'联系客户',route:'w-appointment',word:'联系'};
  ...
  const servingProgress = step => `<div class="w-serving-progress"><span class="w-serving-stage"><strong>${step+1}<small>/3</small></strong><span><small>当前步骤</small><b>${taskSteps[step]}</b></span></span><span class="w-serving-enter">继续处理 <b aria-hidden="true">›</b></span></div><div class="w-serving-progress-track" aria-hidden="true">${taskSteps.map((_,i)=>`<i class="${i<=step?'filled':''}"></i>`).join('')}</div>`;
  function servingCard(t) {
    const step=t.resumeStep||0;
    return `<article class="w-serving" aria-label="正在服务 · ${e(t.name)}"><button type="button" class="w-serving-hitarea" data-worker="resume-service" data-value="${e(t.id)}" aria-label="继续处理${e(t.name)}的任务"></button><div class="w-serving-head"><span><i aria-hidden="true"></i>正在服务</span><small>${e(t.type)} · ${e(t.id.slice(-6))}</small></div><div class="w-serving-main"><h3>${e(t.name)}<span>${e(t.product)}</span></h3><p>${e(t.address)}</p></div>${servingProgress(step)}</article>`;
  }
  function servingGroup(tasks) {
    return `<article class="w-serving w-serving-group" aria-label="${tasks.length} 单正在服务"><div class="w-serving-head"><span><i aria-hidden="true"></i>正在服务</span><small>${tasks.length} 单需要继续处理</small></div>${tasks.map(t=>{const step=t.resumeStep||0;return `<section class="w-serving-item" aria-label="正在服务 · ${e(t.name)}"><button type="button" class="w-serving-hitarea" data-worker="resume-service" data-value="${e(t.id)}" aria-label="继续处理${e(t.name)}的任务"></button><div class="w-serving-item-title"><h3>${e(t.name)}<span>${e(t.product)}</span></h3><small>${e(t.type)} · ${e(t.id.slice(-6))}</small></div><p>${e(t.address)}</p>${servingProgress(step)}</section>`;}).join('')}</article>`;
  }
  ...
  function renderTasks() {
    ...
    const appointmentsToday=confirmedAppointments(TODAY).length;
    const completedThisMonth=visibleTasks().filter(t=>['followup','closed'].includes(t.queue)&&t.completedAt?.startsWith(TODAY.slice(0,7))).length;
    const overview=`<section class="w-task-overview" aria-label="工作概览"><div class="w-task-overview-summary"><span>待处理任务</span><div class="w-task-overview-total" aria-label="待处理任务 ${pending.length} 单"><strong>${pending.length}</strong><small>单</small></div></div><div class="w-task-overview-actions" aria-label="任务快捷入口"><button type="button" class="w-task-overview-action" data-go="w-schedule" aria-label="查看今日已确认预约日程" title="今日预约"><img src="assets/icons/calendar.svg" alt=""></button><button type="button" class="w-task-overview-action" data-go="w-map" aria-label="查看待处理任务地图" title="任务地图"><img src="assets/icons/map.svg" alt=""></button></div><div class="w-task-overview-stats" aria-label="今日已预约 ${appointmentsToday} 单，本月已完成 ${completedThisMonth} 单"><span>今日已预约 <strong>${appointmentsToday}</strong> 单</span><span>本月已完成 <strong>${completedThisMonth}</strong> 单</span></div></section>`;
    const taskFocus=active.length?`<section class="w-serving-area" aria-label="当前服务任务">${active.length===1?servingCard(active[0]):servingGroup(active)}</section>`:overview;
    return `${taskFocus}
      ...
      ${state.queue==='pending'&&pending.some(t=>workGroup(t)==='OVERDUE')?act(`预约时间已过 ${pending.filter(t=>workGroup(t)==='OVERDUE').length} 单 · 需核对安排　${state.filter==='overdue'?'返回全部':'查看 ›'}`,'filter',state.filter==='overdue'?'all':'overdue','w-overdue-entry'):''}
      ...
      <div class="w-list-heading"><span>${state.filter==='all'?'全部':{overdue:'需核对安排',today:'今日预约',returned:'待补资料',parts:'等配件',processing:'后续跟进',closed:'已关闭',cancelled:'已取消'}[state.filter]} · ${matched.length} 单 ${state.filter!=='all'||state.search?act('清除筛选','clear-filter','','w-text-button'):''}</span><small>按处理顺序</small></div>
      ${promoted?`<p class="w-promoted-note">其中 ${promoted} 单正在服务，已在上方展示</p>`:''}
      ${list.map((t,i)=>(state.queue==='pending'&&(i===0||workGroup(t)!==workGroup(list[i-1]))?`<div class="w-list-heading">${workGroupLabels[workGroup(t)]}</div>`:'')+taskCard(t)).join('')||(!promoted?empty('这个队列暂时没有任务','试试其他状态或清除查询条件。'):'')}`;
  }
  function confirmedAppointments(date){return visibleTasks().filter(t=>inQueue(t,'pending')&&t.flag==='已预约'&&t.date===date).sort((a,b)=>a.start.localeCompare(b.start));}
  const scheduleDate=value=>new Date(`${value}T12:00:00`);
  const scheduleKey=date=>`${date.getFullYear()}-${String(date.getMonth()+1).padStart(2,'0')}-${String(date.getDate()).padStart(2,'0')}`;
  const scheduleLabel=date=>`${date.getMonth()+1} 月 ${date.getDate()} 日`;
  function schedule(){
    const selected=scheduleDate(state.scheduleDate),first=new Date(selected);
    first.setDate(first.getDate()-(first.getDay()+6)%7);
    const days=Array.from({length:7},(_,i)=>{const day=new Date(first);day.setDate(day.getDate()+i);return day;});
    const appointments=confirmedAppointments(state.scheduleDate),weekdays=['一','二','三','四','五','六','日'];
    const week=`${scheduleLabel(first)} — ${scheduleLabel(days[6])}`;
    return `<section class="w-schedule-calendar" aria-label="任务周历"><div class="w-schedule-calendar-head"><div><small>待办事项 · 任务日程</small><strong>${e(week)}</strong></div><div class="w-schedule-calendar-controls">${act('‹','schedule-week','-7','w-schedule-week-arrow')}${act('今天','schedule-today','','w-schedule-today')}${act('›','schedule-week','7','w-schedule-week-arrow')}</div></div><div class="w-schedule-week" aria-label="选择日期">${days.map((day,i)=>{const key=scheduleKey(day),selected=key===state.scheduleDate,today=key===TODAY,count=confirmedAppointments(key).length;return `<button type="button" class="w-schedule-day ${selected?'selected':''}" data-worker="schedule-day" data-value="${key}" aria-label="${key}${today?' 今天':''}，${count} 单预约" aria-pressed="${selected}"><span>周${weekdays[i]}</span><strong>${day.getDate()}</strong><i class="${count?'has-plan':''}" aria-hidden="true"></i></button>`;}).join('')}</div></section><div class="w-schedule-section-head"><div><small>${e(scheduleLabel(selected))} · 星期${weekdays[(selected.getDay()+6)%7]}</small><strong>每日待办</strong></div><span>已确认 ${appointments.length} 单</span></div><div class="w-schedule-list">${appointments.map(t=>`<button class="w-schedule-item" type="button" data-worker="task" data-value="${e(t.id)}"><span class="w-schedule-time"><strong>${e(t.start)}</strong><small>至 ${e(t.end)}</small></span><span class="w-schedule-detail"><small>已确认预约</small><strong>${e(t.name)} · ${e(t.type)}</strong><small>${e(t.address)}</small></span><span class="w-schedule-chevron" aria-hidden="true">›</span></button>`).join('')||empty('当天暂无已确认预约','选择其他日期查看日程；待确认时间的任务仍在任务列表中。')}</div>${go('返回任务','w-tasks','w-text-button')}`;
  }
  function taskMap(){const points=visibleTasks().filter(t=>inQueue(t,'pending')&&t.type!=='远程指导'&&t.address);return `<div class="w-destination-head"><span>本人待处理任务</span><strong>任务地图 <b>${points.length}</b> 个位置</strong></div><div class="w-map-preview" role="img" aria-label="待处理任务位置示意图"><span class="w-map-road road-one"></span><span class="w-map-road road-two"></span><span class="w-map-road road-three"></span><span class="w-map-block block-one"></span><span class="w-map-block block-two"></span><span class="w-map-block block-three"></span>${points.slice(0,6).map((t,i)=>`<span class="w-map-pin pin-${i+1}" title="${e(t.name)}">${i+1}</span>`).join('')}</div><p class="w-map-caption">示意位置 · 点击下方任务查看详情</p><div class="w-map-list">${points.map((t,i)=>`<button type="button" class="w-map-item" data-worker="task" data-value="${e(t.id)}"><span class="w-map-index">${i+1}</span><span><strong>${e(t.name)} · ${e(t.type)}</strong><small>${e(t.address)}</small></span><b aria-hidden="true">›</b></button>`).join('')||empty('暂无可展示的上门任务','远程指导任务不显示在地图中。')}</div>${go('返回任务','w-tasks','w-text-button')}`;}
  function detail() {
    ...
    return `<div class="w-status ${tone(t)}"><small>${queueName(t)}</small><h2>${e(nextStep(t).label)}</h2><p>${e(nextStep(t).hint)}</p></div>${summary(t)}
      ${card(`<div class="w-section-head"><h3>${canArrange(t)?'服务安排':'服务记录'}</h3>${canArrange(t)?go('联系 / 改约','w-appointment','w-text-button'):''}</div><small class="muted">${e(appointmentLabel(t))}</small><div class="w-appointment-time">${t.date?e(`${t.date} · ${t.start}–${t.end}`):'尚未确认时间'}</div><p>${e(t.address)}</p><small class="muted">${e(t.supplement)}</small>${canArrange(t)?`<div class="w-contact">${act('拨打客户电话','phone',t.id)}${go('编辑补充信息','w-edit','w-text-button')}</div>`:''}`)}
  ...
  function requisition(){
    const d=draft('w-requisition'),linked=related(),purpose=linked?'work-order':state.requisitionPurpose;
    const choice=(value,title)=>act(title,'requisition-purpose',value,purpose===value?'selected':'');
    const options=visibleTasks().filter(t=>!['closed','cancelled'].includes(t.queue));
    const workOrderField=linked?`<div class="w-requisition-order"><div class="w-requisition-order-head"><strong>关联工单</strong>${act('返回工单','linked-task','','w-text-button')}</div><div class="w-requisition-value">${e(linked.id)} · ${e(linked.name)}</div><input type="hidden" name="workOrder" value="${e(linked.id)}"></div>`:`<div class="w-requisition-order"><div class="w-requisition-order-head"><strong>关联工单</strong></div><label class="w-requisition-select"><select name="workOrder" aria-label="选择本人可见工单" required><option value="">请选择本人可见工单</option>${options.map(t=>`<option value="${e(t.id)}" ${d.workOrder===t.id?'selected':''}>${e(`${t.id} · ${t.name} · ${t.type}`)}</option>`).join('')}</select><i aria-hidden="true"></i></label></div>`;
    const purposeCard=card(`<h3>领料用途</h3>${linked?'<div class="w-requisition-locked"><strong>工单领料</strong><small>领料用途已锁定</small></div>':`<div class="w-requisition-choices" data-worker-purpose>${choice('work-order','工单领料')}${choice('stock','常备库存补充')}</div>`}${purpose==='work-order'?workOrderField:'<p class="w-requisition-hint">补充个人库存，审批通过后可用于工单服务。</p>'}`,'w-requisition-purpose');
    const available=parts.filter(p=>state.stationInventory[p.id]>0),selected=available.filter(p=>state.basket[p.id]>0),total=selected.reduce((sum,p)=>sum+state.basket[p.id],0);
    const rows=available.map(p=>`<div class="w-requisition-row"><div class="w-requisition-row-top"><strong>${p.name}</strong><span>可领 ${state.stationInventory[p.id]}</span></div><div class="w-requisition-row-bottom"><small>${p.model}</small><div class="w-requisition-stepper"><button type="button" data-worker="basket-minus" data-value="${p.id}" aria-label="减少 ${p.name}" ${!state.basket[p.id]?'disabled':''}>−</button><b>${state.basket[p.id]||0}</b><button type="button" data-worker="basket-plus" data-value="${p.id}" aria-label="增加 ${p.name}" ${state.basket[p.id]>=state.stationInventory[p.id]?'disabled':''}>+</button></div></div></div>`).join('');
    const review=selected.length?card(`<div class="w-requisition-review-head"><strong>本次领料</strong><b>${selected.length} 种 · ${total} 件</b></div>${selected.map(p=>`<div class="w-requisition-review-row"><span>${p.name}</span><strong>× ${state.basket[p.id]}</strong></div>`).join('')}`,'w-requisition-review'):'';
    const noteField=card(`<label class="w-requisition-field"><span>${purpose==='work-order'?'领料说明':'补充原因'}</span><textarea name="note" rows="3" required placeholder="${purpose==='work-order'?'请填写本次领料说明':'请填写常备库存补充原因'}">${e(d.note||'')}</textarea></label>`,'w-requisition-info');
    return taskLink()+form('requisition',`<input type="hidden" name="purpose" value="${purpose}">${purposeCard}<div class="w-requisition-list-head"><h3>选择配件</h3><small>${serviceStation.name}</small></div>${available.length?`<div class="w-requisition-list">${rows}</div>`:empty('本站暂无可领配件','当前服务站暂无可领库存，请联系服务站管理员。')}${review}${available.length?noteField:''}`,purpose==='work-order'?'提交领料申请':'提交补充申请');
  }
  ...
  const rootNavImplementationNote='【小程序开发约束】本页是底部 Tab 根页面，真实小程序需在 pages.json 对本页配置 navigationStyle: custom。顶部不得照搬原型 92px，必须结合 uni.getWindowInfo() 的 statusBarHeight / safeArea 和微信 wx.getMenuButtonBoundingClientRect() 运行时计算导航高度及右侧避让，保留胶囊安全区。页面在顶部时使用无白底的展开标题，内容上滑后切换为白色表面与紧凑页标题，回到顶部后恢复。';
  const screens=[];
  ...
  screen('w-tasks','任务','W01','w-tasks',renderTasks,null,'用处理优先级、预约时间和下一步识别任务，三个 Tab 保留全部业务入口。','顶部概览以待处理总数为主视觉，次级展示今日已确认预约和本月通过审核的本人完成数；今日预约进入日程，任务地图进入本人待处理任务点位。本月完成数表示服务人员本次处理结束，不表示回访等后续流程已完成。待处理按今日已确认、待联系、未来预约、等待处理分组；过时预约提供核对入口。正在服务时置顶展示当前客户、步骤与继续入口，多单保留各自入口。',taskRootNav);
  screen('w-schedule','日程','W01','w-tasks',schedule,null,'按周切换日期，查看本人每日已确认预约并进入任务详情。','周历从周一到周日；仅以已确认预约计数，演示今日为 2026-09-13。');
  screen('w-map','任务地图','W03','w-tasks',taskMap,null,'按位置查看本人待处理的上门任务并进入详情。','地图为原型示意，不显示路线、距离或预计到达；真实小程序使用微信内置地图。');
  ...
  function actAction(action,value){
    ...
    switch(action){
      ...
      case 'schedule-day':if(/^\d{4}-\d{2}-\d{2}$/.test(value))state.scheduleDate=value;return 'w-schedule';
      case 'schedule-week':{const next=scheduleDate(state.scheduleDate);next.setDate(next.getDate()+(value==='-7'?-7:7));state.scheduleDate=scheduleKey(next);return 'w-schedule';}
      case 'schedule-today':state.scheduleDate=TODAY;return 'w-schedule';
      ...
      case 'filter':if(state.queue!=='pending')return 'w-tasks';state.filter=['all','today','parts','overdue'].includes(value)?value:'all';state.search='';return 'w-tasks';
  ...
  function simulate(kind){const t=task(),r=record();switch(kind){
    ...
    case 'approved':if(t?.queue!=='review')throw new Error('先提交当前工单的完工资料，再模拟审核结果。');t.queue='followup';t.flag='审核通过';t.completedAt=TODAY;log(t,'服务中心审核通过','师傅本次处理结束，后续由服务中心回访跟进。');return 'w-review';
  ...
  window.TOTO_WORKER={
    ...
    owns:id=>id?.startsWith('w-'),snapshot,visibleTasks,canArrange,isServing,activeTasks,taskCardScenario,setTaskCardScenario,queueName,nextStep,reset,saveListPosition,restoreListPosition,filteredTasks,submitAction,actAction,simulate,saveDraft,overlay,

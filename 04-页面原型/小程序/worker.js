...
(function () {
  ...
  function reset(company='上海示例服务企业') {
    state={company,loggedIn:true,tasks:company==='上海示例服务企业'?seedTasks():[],taskId:'WX202609130018',queue:'pending',search:'',sort:'priority',filter:'all',step:0,listPosition:null,
      drafts:{},inventory:company==='上海示例服务企业'?{P001:2,P002:1,P003:0}:{P001:0,P002:0,P003:0},stationInventory:company==='上海示例服务企业'?{P001:8,P002:12,P003:0}:{P001:0,P002:0,P003:0},basket:{},partId:'P001',partSearch:'',linkTask:null,
  ...
  screen('w-requisition','领配件','W06','w-parts',requisition,null,null,'按当前服务站选择多项配件；工单号从授权任务选择，后台通过后直接完成双方库存转移。');
  screen('w-return','申请退料','W07','w-parts',returnPart);
  screen('w-records','配件记录','W09 / W10 / W11','w-parts',records);
  screen('w-record-detail','配件单详情','W09 / W10 / W11','w-parts',recordDetail,null,null,'领料、退料、调拨共用记录视图；领退料审核由右侧评审控制器模拟，通过即完成，仅调拨保留收件确认。');
  ...
  function addRecord(type,items,note,taskId){const prefix=type==='领料'?'LL':type==='退料'?'TL':'DB',base=`${prefix}${serviceStation.code}${TODAY.replaceAll('-','')}`,seq=state.records.filter(r=>r.id.startsWith(base)).length+1;const r={id:`${base}${String(seq).padStart(3,'0')}`,type,items:items.map(i=>({...i})),note,taskId:taskId||state.linkTask||task()?.id,status:'处理中',source:type==='调拨'?'示例人员库存 → 丁师傅':serviceStation.name,person:'丁师傅',createdAt:'2026-09-13 15:26',history:['申请已提交 · 状态为处理中']};state.records.push(r);state.recordId=r.id;state.recordType=type;state.recordFilter='全部';return r;}
  ...
  function submitAction(kind,v,page){
    ...
    switch(kind){
      ...
      case 'requisition':{requireText(v.workOrder,'请选择工单号。');requireText(v.note,'请填写领料说明。');if(!visibleTasks().some(t=>t.id===v.workOrder))throw new Error('工单号无效或不在当前权限范围。');const items=Object.entries(state.basket).filter(([,q])=>q>0).map(([id,q])=>({id,quantity:integer(q,state.stationInventory[id])}));if(!items.length)throw new Error('请至少选择一件配件。');addRecord('领料',items,v.note,v.workOrder);state.basket={};return 'w-record-detail';}
      case 'return':requireText(v.workOrder,'请选择工单号。');requireText(v.note,'请填写退料说明。');if(!visibleTasks().some(t=>t.id===v.workOrder))throw new Error('工单号无效或不在当前权限范围。');if(state.records.some(r=>r.type==='退料'&&r.status==='处理中'&&r.items.some(i=>i.id===state.partId)))throw new Error('该配件已有待处理退料单，请先查看处理进度。');addRecord('退料',[{id:state.partId,quantity:integer(v.quantity,state.inventory[state.partId])}],v.note,v.workOrder);return 'w-record-detail';
  ...
  function actAction(action,value){
    ...
    switch(action){
      ...
      case 'receive':{const r=state.records.find(x=>x.id===value);if(r?.status!=='待收件'||r.type!=='调拨')throw new Error('当前记录无需确认收件。');r.items.forEach(i=>state.inventory[i.id]+=i.quantity);r.status='已完成';r.history.push('服务人员确认已收到调拨实物 · 个人库存已更新');return 'w-record-detail';}

...
(() => {
  ...
  const step = (number, title, description) => `<div class="c-step-label">${number} / 3 · ${e(title)}</div><header class="c-page-heading"><h2>${e(title)}</h2><p>${e(description)}</p></header>`;
    ...
    screens: [
      ...
      },
      {
        ...
        body: ctx => {
          ...
          return `<div class="c-step-label">2 / 3 · 购买日期</div><header class="c-page-heading"><h2>这件商品，什么时候到家？</h2><p>请按实际购买情况填写日期。</p></header>${selected.length ? `<div class="c-date-product"><img src="${e(selected[0].image)}" alt="${e(selected[0].name)}"><h3>${e(selected[0].name)}</h3><p>${e(selected[0].model)}</p></div>` : registrationSummary([])}<form id="c-purchase-date-form" class="c-form" data-submit-go="c-purchase"><label class="field"><span>购买日期 <em>必填</em></span><input name="purchaseDate" type="date" max="${dateLimit()}" value="${e(registrationField(ctx, 'purchaseDate'))}" required></label><p class="c-hint">可参考购买凭证或订单上的日期。</p></form><button class="c-text-button" data-go="${registration(ctx).method === 'product-code' ? 'c-product-code' : 'c-manual-select'}">重新选择或识别商品</button>`;
        },
      ...
      },
      {
        ...
        body: ctx => `${step(1, type(ctx) === 'repair' ? '哪里需要帮助？' : '想了解怎样使用？', type(ctx) === 'repair' ? '描述一下遇到的情况，我们一起解决。' : '告诉我们您的疑问，选择适合您的指导方式。')}${compact(ctx)}<div class="c-type-options" aria-label="服务类型"><button type="button" data-service-type="repair" data-go="c-repair" aria-pressed="${type(ctx) === 'repair'}">维修</button><button type="button" data-service-type="remote-guidance" data-go="c-repair" aria-pressed="${isRemote(ctx)}">远程指导</button><button type="button" data-service-type="onsite-guidance" data-go="c-repair" aria-pressed="${type(ctx) === 'onsite-guidance'}">上门指导</button></div><p class="c-hint">${isRemote(ctx) ? '通过电话等远程方式沟通，先帮助您了解操作与使用方法。' : type(ctx) === 'onsite-guidance' ? '由服务人员联系您，确认上门指导的实际安排。' : '您提供的信息越清楚，我们越容易了解产品的情况。'}</p><form id="c-repair-form" class="c-form" data-submit-go="c-contact"><label class="field"><span>${type(ctx) === 'repair' ? '问题描述' : '想咨询的问题'} <em>必填</em></span><textarea name="description" rows="4" maxlength="1000" placeholder="${type(ctx) === 'repair' ? '例如：哪项功能不正常，什么时候开始出现？' : '例如：想了解遥控器设置、清洁保养或日常使用。'}" required>${e(field(ctx, 'description'))}</textarea></label><div class="c-field-heading"><strong>照片或视频</strong><span>选填</span></div>${ctx.mediaStatus === 'failed' ? '<div class="c-media-feedback is-failed"><div><strong>这份资料还没有添加成功</strong><p>已填写的内容仍在，可以重试或稍后补充。</p></div><button type="button" class="secondary" data-media="retry">重试</button><button type="button" class="c-text-button" data-media="remove">暂不添加</button></div>' : ctx.mediaStatus === 'uploaded' ? '<div class="c-media-feedback"><div><strong>已添加 1 张示例照片</strong><p>可在提交前移除或重新添加。</p></div><button type="button" class="c-text-button" data-media="remove">移除</button></div>' : '<button class="c-media-add" type="button" data-media="add"><span class="c-inline-icon" data-icon="plus" aria-hidden="true"></span><strong>添加照片 / 视频</strong><small>帮助我们更快了解情况</small></button>'}</form>`,
      ...
      },
      {
        ...
        body: ctx => {
          ...
          return `<div class="c-result-heading"><span class="c-result-mark" data-icon="check" aria-hidden="true"></span><p class="c-kicker">我们已收到您的申请</p><h2>接下来，交给我们联系</h2><p>请保持手机畅通。<br>服务时间和具体安排将在联系后确认。</p></div><div class="c-receipt-status"><span class="c-status-dot" aria-hidden="true"></span><strong>等待联系确认</strong><span>申请已提交</span></div><section class="c-section"><h3>本次申请</h3><dl class="c-details">${row('服务产品', order.productName || item(ctx).name)}${row('服务类型', types[order.serviceType] || typeName(ctx))}${row('申请单号', order.id)}${row('联系人', `${order.contactName || ''} · ${order.phone || ''}`)}${row(order.serviceType === 'remote-guidance' ? '期望联系时间' : '期望上门时间', `${order.preferredDate || ''} ${times[order.preferredTime] || order.preferredTime || ''}`)}</dl></section><p class="c-hint">您可以随时在“服务”中查看这次申请的进度。</p>`;
        },
